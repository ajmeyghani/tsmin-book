...
\subsection{Class Decorator}\label{class-decorator}

Class decorators are used to decorate classes.

\subsection{Property Decorators}\label{property-decorators}
...
\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\FunctionTok{@component}\NormalTok{(\{ }\KeywordTok{... }\NormalTok{\});}
...
\item
  The most notable ones are the \emph{LifeCycle Hooks}
\item
  Angular defines the interfaces and you have to provide the
  implementation
\item
  Angular defines the \texttt{lifeCyle} interfaces in the \texttt{link}
  folder:
\end{itemize}
...
Below is an example of using the \texttt{onInit} hook:

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\KeywordTok{import \{bootstrap\} from 'angular';}
\KeywordTok{import \{onClick\} from 'angular';}
\FunctionTok{@component}\NormalTok{(\{}
  \NormalTok{selector: 'app'}
\NormalTok{\});}
\KeywordTok{class} \NormalTok{App }\KeywordTok{implements} \NormalTok{onClick \{\}}
\end{Highlighting}
\end{Shaded}
